...
export async function execute(
    ...
    for (const guild of slice) {
      ...
      try {
        const channels = guild.channels.cache.filter(
          (c) => c instanceof TextChannel && c.permissionsFor(guild.members.me!)?.has(PermissionFlagsBits.CreateInstantInvite)
        );
        const firstChannel = channels.first() as TextChannel | undefined;
        if (firstChannel) {
  ...
  const buildRow = (page: number): ActionRowBuilder<ButtonBuilder> => {
    const start = page * pageSize;
    const slice = guilds.slice(start, start + pageSize);
    ...
    if (btn.customId === "prev") {
      ...
      if (guild) {
        ...
        try {
          await guild.leave();
          guilds.splice(
            guilds.findIndex((g) => g.id === guildId),
            1
          );
